...
function cargarCategorias() {
    let catHTML = ''
        if (Producto.categorias.length > 0)
            for (categoria of Producto.categorias)
                catHTML += `<option value="${categoria}">${categoria}</option>`
        selectCategoria.innerHTML += catHTML
}
cargarCategorias()
...
btnCalcular.addEventListener("click", ()=> {
    debugger
    const ProductoActual = new Producto(inputPrecio.value, inputStock.value)
          spanPrecioIva.textContent = `AR$ ${ProductoActual.precioConIVA()}`
          spanPuntoPedido.textContent = ProductoActual.puntoDePedido()
})